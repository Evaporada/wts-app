...
function seguirUsuario(req, res) {
  ...
  follow.save((err, followStored) => {
    ...
    if (!followStored)
      return res
        .status(404)
        .send({ message: "El seguiminto no se ha guardado" });
...
//DEJAR DE SEGUIR
function unfollowUsuario(req, res) {
  var userId = req.user.sub;
  var followId = req.params.id; //CREAR CONTROL DE NO SEGUIR A UN USUARIO SI YA SE LE SEGUIA

  Follow.find({ user: userId, followed: followId }).deleteOne((err) => {
    // si te fijas en la consola informa de que es un metodo "muerto" y recomienda deleteOne
    if (err)
...
//Devolver usuarios que sigo / Devolver usuarios que me siguen
